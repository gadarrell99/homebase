...
async def api_credential_scan(request: Request):
    """Return latest credential scan — requires vault token"""
    auth = request.headers.get("Authorization", "")
    token = auth.replace("Bearer ", "")
    ...
    try:
        ...
        return {"error": "No scan data yet — scanner runs hourly"}


@app.get("/api/cheat-sheet-full")
async def api_cheat_sheet_full(request: Request):
    """Full infrastructure data — requires vault token. Reads from infrastructure.json."""
    auth = request.headers.get("Authorization", "")
    token = auth.replace("Bearer ", "")
    from services.totp_service import verify_token
    if not verify_token(token):
